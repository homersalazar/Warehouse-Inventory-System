...
@section('content')
    <div class="flex justify-center items-center w-full pt-12">
        <form action="{{ route('register') }}" method="POST" class="w-72">
            @csrf            
            <h1 class="text-2xl mb-6">Sign Up</h1>
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="fullname" id="fullname" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            ...
            <div class="mt-6">
                Already have an account? <a href="{{ route('user.login') }}" class="text-blue-600 hover:underline">Sign in</a>
            </div>
        </form>
    </div>
@endsection