...
@section('content')
    <div class="flex flex-col md:items-center pt-5 px-5 sm:ml-9">
        <h1 class="text-left text-lg sm:text-2xl font-bold">New Unit</h1>
        <form method="POST" action="{{ route('unit.store') }}">
            @csrf
            <div class="mb-6 mt-2">
                <label class="block mb-2 text-sm font-medium">Name <span class="text-red-600">*</span></label>
                <input type="text" name="unit_name"  class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full sm:w-96 p-2.5" placeholder="Unit of measure" required>
            </div>
            <div>
