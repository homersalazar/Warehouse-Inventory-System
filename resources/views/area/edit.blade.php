...
@section('content')
    <div class="flex flex-col md:items-center pt-5 px-5 sm:ml-9">
        <h1 class="text-left text-lg sm:text-2xl font-bold">Editing {{ $area->area_name }}</h1>
        <form method="POST" action="{{ route('area.update', $area->id) }}">
            ...
            <div class="mb-6 mt-2">
                ...
                <input type="text" name="area_name" value="{{ $area->area_name }}" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full sm:w-96 p-2.5" placeholder="Area name" required>
            </div>
            <div>
